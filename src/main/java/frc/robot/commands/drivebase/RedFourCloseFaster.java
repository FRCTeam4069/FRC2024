...
public class RedFourCloseFaster extends SequentialCommandGroup {
    private SwerveDrivetrain drive;
    private IntakeController intake;
    private IndexerController indexer;
    private ShooterController shooter;
    private ShooterRotationController rotShoot;
    public RedFourCloseFaster(SwerveDrivetrain drive, IntakeController i, IndexerController index, ShooterController shooter, ShooterRotationController rot) {
        this.drive = drive;
        intake = i;
        indexer = index;
        ...
        addCommands(
            new SequentialCommandGroup(
                ...
                new ParallelCommandGroup(
                    ...
                    new SequentialCommandGroup(
                        new FollowPath(drive, "red close four ring p1"),
                ...
                new ParallelDeadlineGroup(
                    //new AutoSetIntakeState(intake, frc.robot.commands.AutoSetIntakeState.State.ON),
                    new SequentialCommandGroup(
                        new FollowPath(drive, "red close four ring p2", 0.2),
                        new FollowPath(drive, "red close four ring p3", new PIDConstants(0.6), 0.0),
                        new InstantCommand(() -> index.setCustomSpeed(0.90))
                    ),
                    new SequentialCommandGroup(
                        // new CustomShooterCommand(rot, shooter, 70, 49, 0.75, 1.5, 2.5).withTimeout(3),
                ...
                new ParallelDeadlineGroup(
                    //new AutoSetIntakeState(intake, frc.robot.commands.AutoSetIntakeState.State.ON),
                    new SequentialCommandGroup(
                        new FollowPath(drive, "red close four ring p4", 0.5),
                        new FollowPath(drive, "red close four ring p5", new PIDConstants(0.9), 0.0),
                        new InstantCommand(() -> drive.stopModules())
                    ),
                    new SequentialCommandGroup(
                        new WaitCommand(0.5),
                ...
                new ParallelCommandGroup(
                    ...
                    new SequentialCommandGroup(
                        new FollowPath(drive, "red close four ring p6", new PIDConstants(1.2), 0.1),
                        new InstantCommand(() -> drive.stopModules())
                    )
                ),
